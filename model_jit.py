...
def JiT_Flowers(**kwargs):
    """
    JiT Small variant (~50.4M parameters)
    Optimized for Oxford Flowers @ 128x128
    """
    return JiT(
        depth=12,            # Maintain depth for complex flower textures
        hidden_size=384,      # Reduced from 768 to significantly cut params
        num_heads=8,          # 512 / 8 = 64 head_dim (standard for efficiency)
        bottleneck_dim=48,    # Half of Base to match reduced hidden_size
        in_context_len=16,    # Reduced as we have fewer classes (102 vs 1000)
        in_context_start=4,   
        patch_size=8,        # 16x16 patches
        **kwargs
    )

def JiT_Celeba(**kwargs):
    """
    JiT Small variant (~32.6M parameters)
    Optimized for CelebA @ 64x64
    """
    return JiT(
        depth=12,
        hidden_size=384,
...
JiT_models = {
    ...
    'JiT-H/16': JiT_H_16,
    'JiT-H/32': JiT_H_32,
    'JiT-Flowers': JiT_Flowers,
    'JiT-Celeba': JiT_Celeba,
    'JiT-CelebaB': JiT_CelebaB
}
